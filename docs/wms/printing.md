...
The worker performs a complete preflight before printing: it verifies the Garland/Annagem order, re-fetches and recalculates the exact order's pallet count through the Teamship API, confirms the local CUPS queue, and verifies the exact Teamship BOL and outbound-label printer options. It repeats the API pallet-count check immediately before outbound labels, then prints one picking list locally, submits one BOL through Teamship, and submits outbound labels equal to the approved pallet count. It reselects and reads back `BIXOLON SRP-770III` on every order immediately before the label action. A failed, expired, crashed, or partially completed job is never retried automatically.
