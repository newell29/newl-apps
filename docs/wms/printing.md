...
Browser execution uses the documented Teamship shipping-order detail host, `https://members.fulfillit.io`. The live signed-in page is authoritative for printing because Teamship API list or detail pallet aliases can be stale. The page reader accepts one unambiguous hidden count, sums bounded observed editable pallet rows, or reads the bounded static pallet table that Teamship renders for completed orders. Missing, ambiguous, invalid, and default-only rows fail closed.
