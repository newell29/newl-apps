...
The signed-in **Open** dashboard is backed by the same dashboard endpoint with `statusSearch=requested`. The endpoint follows Teamship's current Syncfusion contract: a JSON `POST` body containing `requiresCounts`, `search`, `skip`, `take`, and `statusSearch`. Teamship regenerates its web session during login, so Newl Apps loads the authenticated shipping-order page after sign-in and sends its current CSRF token and refreshed cookies with the dashboard POST. For targeted Garland review, Newl Apps uses Teamship's server-side Garland search on this Open view, caps a dashboard page at 100 rows, and then confirms exact PS/SR pairs deterministically. This avoids exhaustive traversal of unrelated active orders. The broad active API list remains a compatibility fallback only when the dashboard request fails and is capped at 1,000 rows.
