...
- A populated inventory Search field does not prove that Teamship applied the query. The guarded reader must wait for the initial inventory grid, type the query with paced keyboard events, activate Teamship's visible Search control, and wait until a visible result surface contains only groups matching the query. Requiring the whole result surface prevents a requested SKU or LPN already visible on the initial unfiltered page from being mistaken for refreshed evidence. Teamship can show a transient `0 of 0` state before valid rows arrive, so an empty state is accepted only after the filtered-surface wait expires. Enter alone and transient, stale, or hidden grids must never be treated as zero inventory.
...
- Ship by LPN can present the LPN and location in a group caption while the SKU, quantity, serial, warehouse, quarantine, and status remain in a separate detail row. Preserve and pair both surfaces; absence of a normal LPN cell is not evidence that the LPN is missing.
- A filtered Ship by LPN result can span several pages. Expand the filtered grid to Teamship's read-only 100-items view, then read any remaining pages before calculating a total or saying an LPN/serial is absent. The guarded reader stops at 25 pages and fails closed if the pager is malformed or larger than that bound.
- Route a request for LPNs, locations, serials, or quarantine details to Ship by LPN even when the search identifier is a SKU. Route an exact serial-number request as `SERIAL`; do not reinterpret the serial as an LPN.
- The Newl Apps caller waits up to two minutes for a supervised browser result, while the worker claim remains valid for three minutes. A longer browser run fails closed instead of returning a late result to a different request.
