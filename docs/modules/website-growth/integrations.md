...
- The weekday executor uses `openai/gpt-5.4-mini` for native OpenClaw tool orchestration. Before the turn starts, the wrapper requires an effective, healthy OpenAI OAuth profile; an API-key profile is never accepted as fallback for Website Growth. The content and final backlink research sessions use Codex `gpt-5.6-sol` after `codex login status` confirms ChatGPT authentication.
