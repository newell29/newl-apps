...
- SEMrush uses `https://mcp.semrush.com/v1/mcp` through official OAuth in the Codex runtime. Newl Apps does not hold the OAuth token and stores only capped, sanitized evidence rows under the existing SEMrush data source with `transport: official_mcp_oauth` metadata. The weekly Position Tracking snapshot is retained as an eight-day cache with its original observation and expiry timestamps; cache reuse creates an audit import but no duplicate metric rows.
