...
- The weekday runner uses Teams' native document attachment support to deliver the SEO performance workbook and, when needed, the SEMrush keyword-import workbook to the same configured target. Safe duplicate-run and failure messages use that same fixed target; Codex cannot alter it.
