...
- Search Console and GA4 use the existing server-side Google API credentials and save tenant-scoped metrics. Search Console requests query/page and page/country reports for both the current and previous complete 28-day periods, excludes the newest two days, and aggregates pages through the website redirect inventory. Query text is deterministically classified for customer-question intent; no additional Google permission is required.
...
- SEMrush remains optional through `https://mcp.semrush.com/v1/mcp` and official OAuth in the Codex runtime. Newl Apps does not hold the OAuth token. As the API-unit fallback, Microsoft Graph application access reads only the `Inbox/Semrush` child folder in `partnerships@newlgroup.com`; only messages whose sender address exactly matches `mail@semrush.com` and eligible PDF attachments are accepted. The importer caps messages, attachments, file size, extracted text, and retained reports. It stores only a content hash, sanitized subject, report date/type, bounded metrics, and sanitized excerpt in the tenant-scoped import ledger. Raw email, Graph message/attachment IDs, and PDF bytes are not stored.
- Microsoft Teams delivery runs through the configured OpenClaw Teams account. Newl Apps constructs the message and review links deterministically; Codex cannot choose a recipient or send a message. Every normal summary begins with the specific workflow and outcome, followed by `Result` and `Action required`, so a lightweight check-in, content review, backlink search, outreach cycle, and page build cannot be mistaken for one another.
...
- Scheduled-report ingestion uses the existing Microsoft Graph application credentials and mailbox access policy. Optional overrides are `WEBSITE_GROWTH_SEMRUSH_MAILBOX` and `WEBSITE_GROWTH_SEMRUSH_MAIL_FOLDER`; defaults are `partnerships@newlgroup.com` and `Inbox/Semrush`. No additional production secret is required when that mailbox is already in the application access scope.
- Scout machine routes use the dedicated `OPENCLAW_WEBSITE_GROWTH_TOKEN` and configured tenant slug.
- Backlink discovery reuses Hunter's existing `HUNTER_BRAVE_SEARCH_API_KEY` and safe public-HTTPS retrieval code. The worker reads only that key from the protected Hunter environment when it is not already present in Scout's environment; it does not import Hunter's other secrets. Search is capped at 12 queries and 120 results in the independent Tuesday backlink run. Codex `gpt-5.4-mini` reviews at most ten candidates per subscription-backed call. An incomplete response retries only omitted IDs, recursively isolates persistent failures, and forwards an unrecoverable individual candidate with zero confidence through the existing bounds. Newl Apps machine routes register canonical URL hashes before page retrieval and return only never-before-seen candidates. Final Codex review receives at most 15 finalists and may promote at most five.
...
- The weekday cron runs `run-website-growth-backlink-executor.sh` as a command job. The wrapper owns the run timestamp, invokes the constrained agent, validates both the required tool sequence and every tool result, calls the tenant-scoped summary endpoint in a separate phase, and delivers Teams without giving the model a messaging tool.
- Initial outreach uses `/api/website-growth/backlinks/executor/send-email`. Due follow-ups use `/api/website-growth/backlinks/executor/send-follow-up`; the latter accepts only opportunity ID and personalized copy, then retrieves the tenant-scoped approved recipient evidence inside Newl Apps.
...
- The weekday executor uses `openai/gpt-5.4-mini` for native OpenClaw tool orchestration. Before the turn starts, the wrapper requires an effective OpenAI profile from OpenClaw's OAuth provider inventory. OpenClaw may expose the valid ChatGPT credential as an `oauth`/`ok` row or a subscription-backed `token`/`static` row; both are accepted, while an API-key profile is never accepted as fallback for Website Growth. The content and final backlink research sessions use Codex `gpt-5.6-sol` after `codex login status` confirms ChatGPT authentication.
