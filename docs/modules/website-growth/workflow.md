...
2. `/api/website-growth/scout/prepare` refreshes Search Console, GA4, and aggregate form evidence. Search Console compares two complete, non-overlapping 28-day periods with a two-day reporting lag, and legacy URLs are combined with their configured redirect destinations before recovery decisions are made. The planner classifies customer-question queries and prepares up to six candidates. Up to two packet positions are reserved for SEO recovery and up to two are reserved for the dedicated question and AI-answer lane.
