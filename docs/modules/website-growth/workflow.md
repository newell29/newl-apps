...
8. OpenClaw sends a Teams outcome for every run, including zero-result runs. Each deterministic message names the exact workflow, states the result and required owner action first, distinguishes work completed from work merely queued, and ends with the review link and next schedule. Content reports contain the page funnel, direct draft links, and seven-day signed Excel links. Backlink discovery reports explicitly say that no outreach was sent and contain searched, duplicate, rejected, finalist, and promoted counts plus `/website-growth/backlinks`.
...
15. A deterministic local notifier checks every two minutes for newly dispatched, Preview-ready, or failed developer builds. Start messages require no action, Preview-ready messages direct the owner to the Vercel preview and draft PR, and failure messages link to the recorded Newl Apps failure. It sends the fixed Newl Apps message to the configured Teams target, acknowledges the exact tenant-scoped event, and never exposes Teams credentials to Vercel, Codex, or Kimi.
