...
4. Local Qwen 3.6 performs bulk snippet triage in batches of at most ten. Incomplete batches retry only omitted candidates and split recursively; an individual candidate that still cannot be classified is excluded safely so backlink triage cannot stop the content-opportunity review. Deterministic code selects no more than 40 public pages, no more than two per domain, for safe non-recursive HTML retrieval. Qwen reviews the retrieved text and returns at most 15 finalists through `/api/website-growth/scout/backlink-discovery/complete`.
5. Codex `gpt-5.6-sol` with high reasoning inspects the current website repository in a read-only, ephemeral session, reviews page candidates, and makes the final backlink decision. It may promote at most five Qwen finalists. Optional SEMrush evidence may be live, an exact fresh cache, or explicitly `UNAVAILABLE`; exhausted units no longer stop the run.
...
14. Tuesday, Thursday, and Friday, `/api/website-growth/scout/check-in` refreshes first-party evidence and queue state, reports the stored SEMrush cache age, and sends Teams status without running Codex or calling SEMrush.
...
9. New contacts are capped at five per rolling day and 20 per rolling week. The executor first syncs replies and opt-outs, then processes due follow-ups and verification, and finally claims newly approved work. Follow-ups are due on days 5 and 12 and close after day 21 without a reply.
