...
4. Local Qwen performs bulk snippet triage. Deterministic code selects no more than 40 public pages, no more than two per domain, for safe non-recursive HTML retrieval. Qwen reviews the retrieved text and returns at most 15 finalists through `/api/website-growth/scout/backlink-discovery/complete`.
