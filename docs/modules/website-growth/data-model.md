...
Paid Phase 1 uses structured attribution on `WebsiteInboundSubmission`, `WebsitePaidCampaignSignal` for durable typed health/opportunity signals, `AutomationJobRun` for hourly and weekly Scout outputs, an empty future-sync `WebsitePaidCampaignMetric` ledger, and an empty future-upload `WebsiteAdsOfflineConversion` audit ledger. Every path is tenant scoped. See `paid-campaigns.md` for the Phase 2/3 boundary.
