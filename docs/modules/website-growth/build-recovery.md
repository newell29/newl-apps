...
- A failed job may move back to `PREVIEW_READY` only when one callback supplies the HTTPS preview URL, HTTPS pull-request URL, matching positive pull-request number, and full 40-character deployment commit SHA.
...
`tests/website-growth-build-completion.test.ts` verifies tenant-scoped job-or-draft lookup, rejects incomplete failed-build recovery evidence, and confirms that complete preview and pull-request evidence updates the existing draft and opportunity.
