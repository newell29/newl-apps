...
Supervisor-correction regressions cover after-hours completion becoming eligible at the next morning wake, recovery of existing waits on read and claim, tenant/revision guards, preserved drafts and feedback, unchanged rolling-budget and active-capacity gates, one-step worker completion, the three-attempt escalation, incomplete review/blocker metadata, missing/partial measurement evidence, and unchanged external/owner/measurement waits. Page-evidence coverage verifies deterministic bounded query totals separately from the visible top-query rows. Handoff coverage changes a terminal page's obsolete build phase and verifies that reconciliation creates no duplicate history. Rendered workboard coverage distinguishes a ready correction from a scheduled review, explains a full budget, and shows the recent selection reason, result, latest wake and next available rolling-budget time. Authority coverage verifies that a manual action remains outside the executor, records a human receipt as SUBMITTED only after fresh tenant-scoped evidence validation, and permits separate placement verification. These checks use synthetic fixtures and do not run research models or contact publishers.
