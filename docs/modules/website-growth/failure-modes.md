...
- Model marks an existing route as a new page: Newl Apps checks the current route inventory, stores the opportunity as an existing-page improvement, and normalizes the page artifact before review. A legacy saved mismatch is reconciled from the final brief mode and disclosed in the UI; it does not become a create-page build.
- Existing-page preview resembles a new-page rewrite: the review screen labels it as an after-change context view, while the explicit current-page change list is the build scope. Scout's quality supervisor returns broad, duplicative rewrites for revision.
- Workboard signal counts differ from a later Search Console comparison: the card labels the saved value as the original research signal and does not present it as current. The dated brief/progress evidence remains authoritative for the current recommendation; missing rows are not converted into current zeroes.
