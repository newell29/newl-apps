...
The workboard shows Needs you today, Scout is working, Waiting on systems and results, a collapsed grouped candidate backlog, and What we are learning. Candidates are research options, not an owner to-do list. Actual budget/active-slot usage explains idle time. The board refreshes every 30 seconds while visible and on window focus, except while a form has focus, so it can reflect build callbacks without disturbing edits.
