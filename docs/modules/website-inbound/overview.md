...
Website-form intake also stores normalized first-party attribution for paid, organic, AI referral, direct, local, and other reporting while preserving the complete incoming request. Test and internal health-check records remain visible for audit but are excluded from marketing calculations. See [attribution contract](attribution-contract.md).

The correspondence pilot adds an opportunity-level Microsoft 365 email history for approved owner mailboxes. Incoming and sent messages synchronize hourly and on demand. Exact conversation and contact-email evidence links mail automatically; ambiguous exact-email matches wait for a person to choose the opportunity. The interface groups distinct Graph messages from the same mailbox conversation into one chronological thread. The assigned owner can prepare an AI-assisted or safe-template draft, edit it, and approve the exact message for sending. No follow-up is sent automatically.
