...
- Website diagnostics and explicitly marked QA/automated checks use Test. Test is a closed workflow status and is retained for form-health audit rather than deleted. Selecting Test manually sets a reversible `MANUAL_TEST_STATUS` exclusion; automated and source-derived exclusions remain durable if the working status later changes.
...
- Website Growth and paid performance calculations require `entryMethod=WEBSITE_FORM`, status other than Test, `isTest=false`, and no marketing exclusion reason. Manual, test, Codex diagnostic, and automated form-check records cannot inflate those aggregates.
