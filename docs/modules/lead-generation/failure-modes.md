...
Hunter retries transient TradeMining network failures and HTTP 429/5xx responses with bounded exponential backoff. Authentication errors, invalid profile filters, and ambiguous lookup values fail immediately and remain visible on the tracked job run. A failed daily query is recovered with the explicit **Run now** action; it is not silently repeated throughout the day.

If TradeMining retrieval completes but Newl Apps ingestion hits an HTTP 408/425/429/5xx response, timeout, connection reset, database-unreachable condition, or exhausted database pool, Hunter does not pay for or repeat the export. Each successful tenant-bound batch is recorded in a mode-`0600` local checkpoint tied to the exact job, profile, CSV digest, and batch layout. The adapter retries the failed request with bounded backoff; the worker may then restart the same adapter once, which resumes at the first unfinished batch. The job output merges later failure updates so the last successful batch, cumulative processed/created/updated counts, checkpoint index, and original diagnostic remain available for Health & Logs. Authentication, validation, checkpoint mismatch, and malformed-response failures do not enter this recovery loop. Once the bounded same-run recovery is exhausted, an operator must explicitly recover the preserved export rather than start another TradeMining query.
...
- Paid retrieval is atomically checkpointed before Luna runs. The checkpoint contains no provider key,
  ...
  and configuration errors do not enter an automatic retry loop. When an automatic retry was not scheduled
...
- **The TradeMining party name contains freight routing text:** resolver version 4 strips trailing `C/O`, `care of`, and `ATTN` segments before Brave research, then spends the bounded search budget on the clean legal/brand identity. The original imported party name remains unchanged for audit.
