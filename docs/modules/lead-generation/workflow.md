...
5. A known Apollo mapping is trusted as the intended account, but its account ID must resolve to Apollo's canonical
   ...
   The explicit employee-resolution action for that state asks for up to three exact Apollo person profile URLs and
   separately authorizes email-only enrichment. This is required when Apollo's private Suggested leads roster is not
   exposed by the supported public organization/domain People Search. Other unresolved matches remain blocked.
