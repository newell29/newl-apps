...
8. Post-research decisions record whether they came from the latest research run or from a still-current carry-forward assessment. Daily Opportunities counts only the latest cohort as today&apos;s research while preserving earlier Hot and Qualified accounts for outreach.

## External signal scout

1. The existing Mac-mini Hunter launch service checks once per local day after `HUNTER_SIGNAL_SCOUT_DAILY_TIME`. The reviewed operating default is enabled at 02:30 America/Toronto so external signals can enter the same morning research cohort; the Hunter policy and kill switch remain authoritative.
...
6. Ollama enforces a JSON schema. The default `qwen3.6:27b-q4_K_M` prompt requires an explicit non-logistics prospect, concrete event, evidence-only summary, service line, and calibrated 0-100 confidence. Missing records and scores below 50 fail closed. Qwen classifies a discovery hypothesis; it cannot authorize Apollo.
...
8. Provisional companies then use normal Brave research, Luna-first synthesis, bounded Qwen 3.6 recovery, Kimi scoring, K3 validation, deterministic eligibility, planning, and assisted outreach handoff. Only a current Hot or Qualified result may reach Apollo contact discovery. No signal-scout path itself performs Apollo search, cadence mutation, enrollment, email, LinkedIn, or other customer communication.
...
1. External scouting is eligible at 02:30, TradeMining profiles at 04:00, and company research no earlier than 05:45 in America/Toronto. Company research additionally waits until every enabled TradeMining profile has finished or failed for the local date, preventing partial same-morning cohorts. The default cohort limit is the saved Hunter policy limit; a new unsaved policy uses 30.
...
5. Luna synthesizes small structured batches through Newl Apps with low reasoning and strict Structured Outputs.
   ...
   accepting it. A failed Luna batch receives bounded backoff and individual-company isolation. Any remaining omission is offered to the installed Qwen 3.6 model using the already-paid evidence checkpoint; valid companies continue.
   ...
   When Luna is enabled, Hunter sends every company's exact bounded evidence packet to a tenant-bound Newl Apps synthesis endpoint in batches of at most four. Newl Apps asks GPT-5.6 Luna to synthesize only the supplied evidence with strict Structured Outputs. Luna remains the preferred synthesis source. Qwen 3.6 is a bounded availability fallback, not a promotion mechanism: deterministic freshness, provider/competitor, identity, and citation corrections still run before Kimi, and K3 still validates the strongest fresh-event candidates. Luna does not browse, repeat Brave retrieval, score, classify, plan, search Apollo, or authorize outreach.
...
9. Daily Opportunities exposes the latest scout counts and a collapsed list of accepted source links. These are clearly labeled as discovery hypotheses and remain separate from Hot, Qualified, Watchlist, and Blocked research results until Luna/Kimi completes. `--company-research-now` runs the queue explicitly. `--company-research-dry-run` stops before persistence. `--company-research-cohort <json>` replays an exact bounded company list for model comparisons. Scheduled and manual runs atomically write a mode-`0600`, cohort-fingerprinted checkpoint immediately after paid retrieval, defaulting under `HUNTER_PROCESSED_DIRECTORY/company-research-checkpoints`. A same-day exact-cohort retry reuses that checkpoint automatically, while `--company-research-output` and `--company-research-resume` remain available for controlled replays. A different prompt version or tenant-prepared cohort fails closed rather than reusing mismatched evidence.
