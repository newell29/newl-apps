...
5. A known Apollo mapping is trusted as the intended account, but its account ID must resolve to Apollo's canonical
   ...
   The explicit **Recheck employees** action for that state is allowed to repeat the read-only, organization-scoped
   lookup even when the latest immutable attempt has a review classification. Other unresolved matches remain blocked.
...
4. **Apollo Exceptions** is the active review queue for fresh Luna/Kimi-vetted Hunter opportunities with ambiguous
   ...
   no longer current; ordinary successful direct matches remain excluded. To resolve it, open the company in
