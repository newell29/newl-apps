...
6. Hunter creates a tracked job run, exports and normalizes the leaf records, and submits tenant-bound batches. After every successful batch, the local worker atomically writes a mode-`0600` checkpoint bound to the tenant-owned job ID, profile ID, batch size, and canonical CSV SHA-256. A transient Newl Apps or database failure receives bounded request retries and one bounded process recovery; recovery reuses the same job and CSV and begins at the first unfinished batch. It never repeats the TradeMining export. A mismatched or unreadable checkpoint fails closed. Coverage metadata reports matches, exported records, query count, completeness, cumulative ingestion counts, and checkpoint progress.
...
8. If a capped result can no longer be divided, Hunter ingests the available export but closes the run as `PARTIAL`. If local profile configuration is invalid, or the bounded same-run ingestion recovery is exhausted, the tracked run finishes as failed and the Search Profiles screen displays the error plus preserved partial counts. Hunter does not start a new TradeMining query for that daily attempt; an operator can correct a permanent problem or explicitly recover the preserved export.
...
2. The worker authenticates through the existing tenant-bound ingestion context. Newl Apps rejects overlap and returns an allowlisted, date-rotated set of discovery topics and geographies, the prior 180-day source-URL ledger, the policy threshold, recommended local model, and no-outreach rules.
3. Brave Web Search is the primary discovery provider. The exact query set changes by local date across distribution facilities, retail rollouts, manufacturing capacity, industrial leases/construction, ecommerce growth, reshoring, North American market entry, import-distribution partnerships, leadership changes, and regional delivery-network events. Geography rotates across the Carolinas/Georgia, the broader Southeast, Ontario/Canada, and North America. Google News RSS is a read-only fallback only when one Brave query returns no usable rows.
4. Discovery remains bounded to 40 selected results per day: 24 warehousing, 12 ocean/air, and 4 trucking. Brave searches use the trailing-month freshness filter. Canonical URLs seen during the prior 180 days and deterministic directory/ranking/roundup title patterns are removed before Qwen, and same-company/signal/geography coverage in one publication month is grouped into one event with up to eight retained public sources.
5. Only HTTPS link, source, publication date, title, bounded search snippet, lens, and service hint are supplied to the local classifier. Article bodies, Apollo data, contacts, and customer records are not sent.
6. Ollama enforces a JSON schema. The default `qwen3:30b-instruct` prompt requires an explicit non-logistics prospect, concrete event, evidence-only summary, service line, and calibrated 0-100 confidence. Missing records and scores below 50 fail closed. Qwen classifies a discovery hypothesis; it cannot authorize Apollo.
7. Newl Apps validates the complete response again. An above-threshold company is matched to the tenant's canonical company identity or created as a tenant-scoped provisional `HUNTER_EXTERNAL_SIGNAL_SCOUT` company even when it has no TradeMining record. At least one third of the next bounded company-research cohort is reserved for the strongest external-signal companies, when available.
8. Provisional companies then use the normal four-pass Brave research, Qwen 3.5 35B synthesis, Kimi scoring, K3 validation, deterministic eligibility, planning, and assisted outreach handoff. Only a current Hot or Qualified result may reach Apollo contact discovery. No signal-scout path itself performs Apollo search, cadence mutation, enrollment, email, LinkedIn, or other customer communication.
9. The run ledger and Teams digest report raw Brave rows, duplicate URLs, selected rows, accepted signals, grouped repeat events, existing companies, and newly promoted companies. `--signal-scout-now` provides an explicit operator rerun. `--signal-scout-dry-run` exercises discovery and classification, then closes the prepared job as intentionally failed without persisting signals.
...
2. The server applies the same customer, pipeline, reply, sequence, do-not-contact, and do-not-prospect exclusions before returning company facts. Automatic selection also suppresses every Apollo-organization/domain/name identity researched during the prior 90 days and any company in an active or paused Hunter cadence. A newer active non-TradeMining expansion, external-news, or manual signal may reopen a company early; another TradeMining shipment refresh cannot. Explicit operator cohorts remain deliberate replays. No contact or Apollo payload is returned.
3. Hunter runs five query passes per company: identity/parent, fresh events, first-party careers, distribution footprint/named external providers, and customs/import records. It searches the legal name plus bounded brand and regional aliases, and adds official-domain identity/event queries when a domain is known. Same-identity TradeMining party names may be used only as bounded search aliases; raw importer, consignee, and shipper names are not model evidence. One fetch slot is reserved for customs results and duplicate URLs are retained only once. The bounded evidence ledger samples results across every enabled query before taking additional results from any one query, so generic search results cannot exhaust the cap before an official-domain fresh-event query runs. After the first synthesis, an ambiguous, below-70%, or uncorroborated identity receives a deterministic domain-discovery recovery pass. Hunter recognizes only domains matching the candidate's bounded aliases, including domains already mentioned inside saved evidence, then searches/fetches official legal, privacy, terms, contact, and about results before rerunning Qwen. Brave Search is the approved production provider; Hunter normalizes Brave's `page_age` as a fallback `publishedAt`, replaces it with the page's original `datePublished` metadata when available, and requires Qwen to cite the exact evidence behind a trigger. DuckDuckGo HTML remains a bounded trial fallback without reliable search-result publication dates.
...
5. Luna synthesizes small structured batches through Newl Apps with low reasoning and strict Structured Outputs.
   Newl Apps validates the provider response and Hunter applies the same evidence-aware field normalization before
   accepting it. Hunter removes a company with zero usable public evidence before calling Luna. A malformed,
   truncated, omitted, or contract-invalid Luna row is retried once as an isolated company request; an exhausted
   company is omitted from that completion while other valid companies continue to Kimi and deterministic gates.
   If no company has a valid Luna synthesis, the complete run fails closed.
   Luna may propose at most two precise follow-up queries per company; Hunter retrieves them and reruns Luna once.
   Follow-up retrieval shares the same hard 24-record company evidence cap, skips unused queries once the ledger is
   full, and bounds the evidence again before completion submission. During the three-run transition, local Qwen
   independently attempts the old synthesis contract as non-blocking comparison telemetry.
   When Luna is enabled, Hunter sends each non-empty exact bounded evidence packet to a tenant-bound Newl Apps synthesis endpoint in batches of at most four. Newl Apps asks GPT-5.6 Luna to synthesize only the supplied evidence with strict Structured Outputs. Luna is authoritative for the Kimi and deterministic path; a missing or invalid Luna row cannot advance and Qwen cannot substitute for it. Qwen may independently process the same packet as a temporary non-blocking shadow. Its row stays server-side and is deliberately withheld from Luna to prevent answer copying. Luna does not browse, repeat Brave retrieval, score, classify, plan, search Apollo, or authorize outreach.
   Automation Settings exposes the comparison ledger, including per-company categorical disagreements, and allows an administrator to replay a Luna audit from the saved evidence packet. Replay does not call Brave and does not rewrite a completed opportunity.
...
9. `--company-research-now` runs the queue explicitly. `--company-research-dry-run` stops before persistence. `--company-research-cohort <json>` replays an exact bounded company list for model comparisons. Scheduled and manual runs atomically write a mode-`0600`, cohort-fingerprinted checkpoint immediately after paid retrieval, defaulting under `HUNTER_PROCESSED_DIRECTORY/company-research-checkpoints`. A transient failure receives at most three worker attempts. A recovery request is tenant-scoped to the preceding failed run, reuses its exact company cohort, and resumes its same-day retrieval or synthesis checkpoint. Preparation failures retry before paid retrieval. Non-transient contract, identity, or configuration failures fail immediately. `--company-research-recovery-run-id <run-id>` explicitly invokes the same guarded recovery path when the original local date and three-attempt budget remain; it does not select a new cohort or repeat a matching paid-retrieval checkpoint. `--company-research-output` and `--company-research-resume` remain available for controlled replays. A different prompt version, date, or tenant-prepared cohort fails closed rather than reusing mismatched evidence.
10. With `HUNTER_TEAMS_TARGET` configured, a completed run reports researched, accepted, blocked, and omitted counts plus the automatic cohort split between new companies, scheduled refreshes, newer-trigger refreshes, recent-repeat suppression, and active-outreach suppression. The Control Tower shows the same selection and recovery state. The digest also adds Luna primary status and coverage plus categorical agreement with the optional Qwen shadow. A transient failure reports its bounded retry and checkpoint stage; only an exhausted or non-transient failure sends the final sanitized failure alert. Raw provider responses, local checkpoint paths, and credentials are never included.
...
2. Newl Apps selects current unresolved Apollo matches attached to an eligible Luna/Kimi Hunter research handoff. It also permits one resolver-version repair for a legacy direct mapping only when it is domainless, scored 19 or lower, and carries the durable mapped-zero-employees marker. Rejected, stale, reviewed, do-not-prospect, and every other already-direct company are excluded.
...
4. The worker runs at most five targeted Brave identity/brand/parent/geography queries and returns at most 20 unique HTTPS title/snippet records. Before spending that budget, deterministic identity cleanup removes freight-party routing suffixes such as `C/O`, `care of`, and `ATTN` and produces legal-name and safe regional-brand aliases. A bounded distinctive-tail recovery query still handles legal-owner and facility names whose operating brand omits a leading qualifier. Resolver version 4 reopens unresolved version-3 fingerprints once so older dirty-name exceptions receive the corrected search plan. Search credentials stay on the Mac.
...
6. The deterministic Apollo resolver runs at most three documented organization searches. It searches the verified official domain and cited operating aliases before repeating the original TradeMining name. Auto-mapping requires a unique direct organization, an independently verified exact official-domain match, and at least 90% public-identity confidence. Every other case remains in Apollo Exceptions with the reason, candidate links, and public evidence.
7. A successful mapping queues only the existing Assisted contact-discovery handoff. When exactly one tenant-scoped company already owns the verified domain and Apollo organization, the resolver reuses that canonical identity. Existing Outreach Plan or cadence history suppresses a second handoff for the duplicate legal alias. It does not approve contacts or plans, spend person-enrichment credits, enroll a cadence, or send communication.
8. `HUNTER_APOLLO_EXCEPTION_AUTOPILOT_ENABLED` defaults off in both the Mac worker and Newl Apps. The worker-side flag prevents the Brave-backed poller and its explicit manual command from starting; the server-side flag remains a second fail-closed boundary. The server enforces `HUNTER_APOLLO_EXCEPTION_DAILY_COMPANY_LIMIT` (default 10, maximum 25), Assisted mode, the Hunter kill switch, a one-job lease, tenant filtering, stale-input revalidation, and an immutable job/audit ledger.
...
5. A known Apollo mapping is trusted as the intended account, but its account ID must resolve to Apollo's canonical
   ...
   alone when available, or with the exact saved account's trusted domain when Apollo exposes no nested ID. When
   Apollo has duplicate saved accounts for the same brand and domain, Hunter also searches a bounded set of their
   distinct nested organization IDs. A final company-keyword People Search is allowed only as candidate generation;
   strict same-brand and exact-domain response validation remains mandatory. Otherwise organization
   discovery is bounded and an immutable match record is saved. An ambiguous or missing latest match stops in Apollo
   Exceptions and blocks automatic repeat lookup. A direct mapped organization that returns zero employees remains
   mapped and moves to a separate employee-lookup section; its direct-match classification does not remove the
   explicit zero-employee follow-up marker, and it is not downgraded into an unmapped-company exception.
   The explicit employee-resolution action for that state may accept up to three exact Apollo person profile URLs and
   separately authorize email-only enrichment as a last-resort diagnostic when Apollo's private Suggested leads
   roster is not exposed by any supported public lookup. A zero result remains parked in the mapped employee-resolution
   section; the company does not require daily remapping. From that section, a reviewer may replace an empty or
   incorrect account with another explicitly confirmed Apollo company URL. The same company can be archived with an
   audited reason such as no usable employees, duplicate company, or irrelevant company. Archiving preserves the
   mapping for deduplication, removes the company from active exceptions, and blocks repeat employee searches until
   the reviewer reopens it. Other unresolved matches remain blocked.
6. Hunter always combines Apollo's saved-contact directory, a 100-result employee search scoped to the canonical confirmed Apollo organization, and an additional organization-scoped relevant-title search. People Search filters are supplied in Apollo's documented URL query format as well as the compatible raw JSON request, preventing a body-only filter from being ignored and then misreported as zero employees. Saved-contact retrieval uses pages of 100 and continues through all matching pages within a 20-page safety cap, rather than treating the first 25 records as complete. The title search runs even when the generic first page already contains acceptable people, uses Apollo's multi-title filter in one request, and includes similar titles. For each of the best 10 masked, role-relevant People Search results, Hunter performs a zero-credit saved-contact search using the person's name, title, and confirmed company. Records merge by Apollo person/contact ID first, then LinkedIn/email, then strict confirmed-company + first-name + title identity; the saved name, email, and contact ID backfill an older masked local record. A legacy Apollo account ID is replaced by the exact nested global organization ID before People Search. When Account View exposes only the saved-account shell, Hunter retries the global organization ID retained from that same reviewer-confirmed mapping; it does not search a parent, sibling, same-domain account, or company keyword. One partial person may not suppress the full employee search. Explicit sibling organizations are rejected. A result must contain a concrete syntactically usable email address before Hunter persists it or submits it for buyer-role review; Apollo's boolean email-availability metadata is insufficient. Unapproved plans still attached to contacts without a concrete email are archived during the recheck. Deterministic ranking builds a 10-person review pool using buyer role, opportunity geography, contactability, prior sequence history, replies, and Apollo's Unresponsive stage. Prior cadence history is ranking context, not a company- or contact-level disqualifier.
7. A low-cost structured buyer-role model compares the complete pool with the exact Hunter service line, opportunity rationale, geography, and recommended persona. It returns `PRIMARY`, `SECONDARY`, `REVIEW`, or `REJECT`, a bounded confidence, responsibility hypothesis, rationale, recommended approach, and risk flags. Model-qualified contacts advance first. An explicit manager-or-higher physical logistics, supply-chain, distribution, warehouse, fulfillment, transportation, shipping/receiving, procurement, purchasing, sourcing, materials, inventory, import, export, or clearly facility-scoped operations role also remains eligible for plan drafting when deterministic contact safety passes. Generic digital, franchise, revenue, people, financial, clinical, sales, marketing, customer-service, or administrative operations roles do not receive that fallback. Clearly individual-contributor coordinator, specialist, analyst, associate, assistant, administrator, clerk, representative, agent, and technician titles cannot be auto-selected solely from a favorable model disposition. An explicit import/export, customs, or trade-compliance specialist is the narrow exception and still requires an independently qualifying model disposition. An explicit geography-mismatch risk blocks automatic selection. Only the best 1-3 contacts are selected.
...
10. Each accepted contact receives a score, three-email Outreach Plan, deterministic grounding check, and model QA check. Only Hot opportunities may also receive a separate call task. Every email ends with the first name of the exact mailbox selected by deterministic Apollo routing. Sender placeholders, generic company signatures, internal evidence IDs, and the Hunter name are forbidden in outbound copy. Before QA, deterministic code normalizes whitespace-damaged evidence references and removes any exact ID from the saved evidence ledger from customer-visible subjects and bodies, then immediately reruns deterministic QA without another model call. Remaining semantic failures may receive up to two bounded complete-sequence model regenerations using the latest exact QA findings; sender, evidence, model-availability, and configuration failures remain human-review items. Drafting and repair prompts explicitly prohibit unsupported assumptions about an existing warehouse process, network, provider, arrangement, capacity, facility, or operating setup and prohibit promises to leave such a setup unchanged. The Outreach Queue labels those three dispositions and provides a bounded bulk **Repair failed QA plans** action. A failure after the bounded retries remains visible and cannot advance or be counted as approval-ready.
...
5. Reproducible evidence-retrieval, handoff, deterministic-rule, and TradeMining code defects can create a tenant-scoped approved Rivet development job only when `HUNTER_RIVET_AUTO_TRIAGE_APPROVAL` exactly equals `OWNER_APPROVED_HUNTER_QUALITY_TRIAGE`. Related findings from one workflow are consolidated into one packet; queued, running, or review-blocked work for that workflow prevents sibling branches.
6. Rivet may inspect the frozen evidence, edit an isolated branch, add tests/docs, and push for independent review. A zero-finding exact-commit `PASS` is required before Rivet opens one draft PR. A blocked review records and reports the branch without creating a PR. Rivet may not reclassify a lead, retry TradeMining or outreach, merge, deploy, write production data, change permissions, or contact a prospect.
7. The daily audit result is sent to Alex through the existing protected Rivet Teams target. It states how many enabled profiles were completed, active, failed, or missing at audit time instead of treating a still-running profile as confirmed complete. Rivet's normal completion/failure message later reports either the reviewed draft PR or the blocked branch. A repeated workflow-scoped defect trips a circuit breaker and is not queued again.
...
4. **Apollo Exceptions** is the active review queue for fresh Luna/Kimi-vetted Hunter opportunities with ambiguous
   ...
   a durable unresolved match-review record and blocks blind automatic reruns. A verified direct-company result with
   that marker stays in the employee-lookup section until a later lookup finds employees or the Hunter opportunity is
   no longer current; ordinary successful direct matches remain excluded. To resolve an unmapped company, paste its
   `https://app.apollo.io/#/accounts/<account-id>` URL. If a confirmed mapping still returns zero employees, open up
   to three relevant people, paste their exact `https://app.apollo.io/#/people/<person-id>` URLs, and authorize the
   bounded email-only lookups. Pasting the company People page again cannot reveal Apollo's private Suggested leads
   roster through the supported public API. A successful employee lookup clears the exception; another zero-contact
   result remains queued for deliberate review. If a current Hunter company has no legacy `Lead`, the authenticated
   reviewer is attached as owner only when they deliberately map, retry, confirm, or reopen the exception.
...
5. URL mapping distinguishes Apollo account links from organization links. An account link is resolved through Account
   ...
   The row leaves the active exception queue only after the latest contact-discovery attempt is a direct match with
   employees; otherwise the saved reason includes saved-contact pages and masked/paid recovery counts. Mapping does
   not enroll a contact in a cadence.
