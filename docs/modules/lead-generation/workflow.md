...
5. Local Qwen 3.5 35B synthesizes small structured batches with thinking disabled. Ollama's schema-constrained generation is validated again by Hunter. If a batch is malformed, truncated, omits a company, or violates a field contract, Hunter records a bounded validation reason, retries each affected company independently up to the configured repair limit, and continues with valid companies instead of discarding the complete cohort. It can propose at most two precise follow-up queries per company; Hunter retrieves them and reruns synthesis once. Follow-up retrieval shares the same hard 24-record company evidence cap, skips unused queries once the ledger is full, and bounds the evidence again before completion submission.
   When the server-side Luna shadow flag is enabled, Hunter sends every company's exact bounded evidence packet to a tenant-bound Newl Apps shadow endpoint in batches of at most four, including companies for which Qwen omitted a valid row. Newl Apps independently asks GPT-5.6 Luna to synthesize only the supplied evidence with strict Structured Outputs. The Qwen row stays server-side and is deliberately withheld from Luna to prevent answer copying; Newl Apps compares the independent results afterward. Qwen remains authoritative for the normal Kimi and deterministic path; Luna does not browse, repeat Brave retrieval, score, classify, plan, search Apollo, or authorize outreach. Its only output is a same-run comparison ledger and completion summary.
   Automation Settings exposes that ledger, including per-company categorical disagreements, and allows an administrator to replay Luna from the saved evidence packet. Replay does not call Brave and remains non-authoritative.
...
10. With `HUNTER_TEAMS_TARGET` configured, a completed run reports researched, accepted, blocked, and omitted counts. When the Luna shadow is enabled, the same digest adds its status, coverage, first-pass schema validity, and categorical agreement with Qwen. A failed scheduled or explicit live run sends a sanitized alert stating that no outreach was sent and whether the paid-retrieval checkpoint can be reused; raw provider responses and credentials are never included.
