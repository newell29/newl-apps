...
10. ambiguous or below-70% identities pivot from candidate-matching domains found in saved evidence to official and legal/about/contact pages, rerun synthesis when first-party evidence is recovered, and never accept an unrelated directory or similar-name domain; the identity pass also retrieves public importer/trade profiles, and only an exact candidate-name plus normalized street-address match between an independent operational trade profile and government registration may substitute for missing first-party identity, while a mismatched address and completely missing trade evidence both fail closed;
