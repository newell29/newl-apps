...
- The default queue excludes do-not-prospect, rejected/disqualified, cashflow-customer, existing-pipeline, replied, previously sequenced, do-not-contact, and companies researched during the prior 90 days. The cooldown compares the Apollo organization, normalized domain, and normalized company-name identities so a renamed or duplicate record cannot consume another research slot. Companies in an active or paused Hunter cadence remain suppressed regardless of age. A newer active non-TradeMining expansion, news, external-scout, or manual signal may reopen a previously researched company before day 90; routine TradeMining shipment refreshes do not bypass the cooldown. An explicit operator replay may name at most 100 company keys, and every key is still resolved inside the authenticated tenant and through the same exclusion rules.
...
- Each automatic research run stores and displays cohort-selection telemetry: selected companies, never-before-researched companies, scheduled refreshes, newer-trigger refreshes, recent repeats suppressed, and active-outreach companies suppressed. The Teams completion message reports the same counts so “accepted companies” and newly created outreach work are not conflated.
