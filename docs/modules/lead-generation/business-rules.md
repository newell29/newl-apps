...
- Every company receives identity/parent, fresh-event, first-party-careers, and distribution-footprint queries. When an official domain is known, its identity and fresh-event queries must execute even when earlier generic searches return enough rows to fill the evidence cap; bounded evidence selection samples every enabled query before filling remaining slots. Local Qwen may request at most two evidence-gap follow-ups. Search results, queries, source URLs/domains, excerpts, pass labels, Brave publication dates, retrieval counts, and failures remain auditable.
