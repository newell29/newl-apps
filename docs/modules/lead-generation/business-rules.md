...
- Every company receives identity/parent, public trade/import identity, fresh-event, first-party-careers, and distribution-footprint queries across the same four research passes. When an official domain is known, its identity and fresh-event queries must execute even when earlier generic searches return enough rows to fill the evidence cap; bounded evidence selection samples every enabled query before filling remaining slots. If Qwen leaves identity below 70%, marks it ambiguous, or lacks corroborating identity, Hunter runs a deterministic brand-domain recovery pass before scoring. It can pivot from a candidate-matching domain already present in saved evidence, searches the brand's official site and first-party legal/about/contact pages, and reruns synthesis when evidence is added. First-party recovery evidence may replace a weaker directory/non-trigger row at the evidence cap. For a company without a first-party site, deterministic identity reconciliation may instead accept a government registration plus an independent public trade profile only when both contain the exact candidate name and same normalized street address and the trade profile explicitly documents import or shipment activity. A registration alone, a trade directory without operational activity, a same-name record at another address, or two records from the same source cannot clear ambiguity. The matched identity pair is pinned into the compact model packet. Local Qwen may request at most two evidence-gap follow-ups. Those follow-ups stop before the shared 24-record company cap, and the completion payload applies the same bound defensively. Search results, queries, source URLs/domains, excerpts, pass labels, Brave publication dates, retrieval counts, and failures remain auditable.
