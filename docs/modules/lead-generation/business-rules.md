...
- `lookbackWindowDays` controls the full trailing TradeMining query window for that individual profile. Worker chunking may split the range into smaller requests but must not shorten it.
