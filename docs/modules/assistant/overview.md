...
- Incorrect or missing Teamship updates cannot be confirmed until an administrator links the exact tenant-scoped saved PS/SR review and its source PDF, or attaches a supporting PDF/image. For an email-backed review, Newl Apps first re-fetches the original attachment using the saved Microsoft Graph identifiers, verifies its PDF signature and parsed content hash, and caches it in existing workflow-artifact storage. A manual attachment is required only when the original email is unavailable, ambiguous, or the issue concerns Teamship UI evidence. This requires no schema change.
...
- Every Rivet branch must pass a fresh read-only Codex review of the exact commit before a PR is opened. Review results are stored in tenant-scoped `CodexReviewRun` records. Only a zero-finding `PASS` for the current commit may create the draft PR and produce `READY_FOR_ALEX`; a blocked review retains the branch and audit record but creates no PR.
- Selecting **Approve & queue Rivet** records the administrator decision and atomically creates a tenant-scoped Rivet development job. When the same module/workflow already has queued, running, or review-blocked work, the new job remains `WAITING_FOR_RIVET`; its approval is not rejected. The restricted local worker claims the oldest eligible job and never runs two jobs from the same module/workflow scope concurrently. It may use Codex to prepare an isolated branch and, after review passes, a draft PR; it cannot merge, deploy, execute a migration, update Teamship, print, release an order, change permissions, or contact a customer.
...
- Alex confirmed that one explicit **Approve & queue Rivet** decision may authorize the restricted Codex branch-and-PR workflow. The job may wait for earlier work in the same scope. Merge and every production or operational action remain separate approvals.
