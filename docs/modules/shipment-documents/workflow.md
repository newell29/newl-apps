...
- Scheduled Garland processing retries only a completely missed batch: the PDF must contain at least one order, every PDF order must be missing from Teamship, and the matched count must be zero. The attachment remains pending for 5 minutes and can be retried up to three times. A partial match is finalized immediately so genuinely missing orders are not hidden or repeatedly queried.
