...
- Carrier-manifest OCR treats `CLARKE` as a target carrier alongside Midland, Speedy, and Suretrack. The employee receives one separate Clarke workbook containing the same SR, PS, city/province, pallet-count, total, and signature layout used for the other carriers.
- Saving a carrier-manifest run stores the Clarke workbook separately and exposes it through tenant-scoped history and download routes.
