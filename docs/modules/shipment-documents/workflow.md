...
- Garland API list/detail reads share a renewable read session across the Open dashboard, bounded legacy fallback, and explicitly requested Complete-archive lookup. A 401 renews the token once for that read and preserves earlier exact matches; subsequent reads use the renewed token. Exhausted authentication recovery or a 403 rejects the comparison, including when only some orders have been read, so an authentication failure cannot become missing-order evidence. This does not retry any Teamship write or change update approval requirements.
