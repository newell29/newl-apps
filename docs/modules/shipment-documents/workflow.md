...
- Graph retry diagnostics are operational evidence only and do not send Teams notifications. Recovered and exhausted reads record the attempt number, Microsoft request ID when supplied, a client correlation ID, timestamp, outcome, and a hashed resource fingerprint instead of raw mailbox/message identifiers.
