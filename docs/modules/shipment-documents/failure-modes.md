...
- Retry evidence is written to application logs and exhausted intake error evidence without a Teams notification. Raw mailbox, message, and attachment identifiers are not added to the structured retry log; a stable hashed resource fingerprint supports correlation without exposing those identifiers.
