...
- A newly parsed batch with zero Teamship matches and all PDF orders missing is treated as a bounded timing-race candidate, not an immediate final failure. Scheduler cycles wait at least 5 minutes between attempts and stop after three retries. Any partial Teamship match bypasses this retry path and is finalized for CSR review.
