...
- Teamship may cap list responses below the requested page size. Exact Garland PS/SR retrieval advances by the actual returned row count and detects repeated pages; a short non-empty page is not treated as the end of the Teamship order list.
- A targeted Teamship lookup that returns no exact PS/SR matches records aggregate diagnostics (requested count, pages and rows scanned, configured limits, and stop reason) without logging customer or order identifiers.
- A transient Microsoft Graph attachment metadata or raw-download failure is retried at most twice after the initial read. Only HTTP 408, 429, 5xx, timeouts, and transport failures qualify. Authorization, validation, and not-found failures are surfaced immediately; exhausted reads remain visible in the intake attachment-error evidence for a later scheduled scan.
- A newly parsed batch with zero Teamship matches and all PDF orders missing is treated as a bounded timing-race candidate, not an immediate final failure. Scheduler cycles wait at least 5 minutes between attempts and stop after twelve retries (a 60-minute window). Any partial Teamship match bypasses this retry path and is finalized for CSR review. If a full miss was finalized before Teamship became ready, an authorized operator can use the saved batch's read-only **Recheck Teamship** action; it refreshes Newl Apps evidence only and never repeats a Teamship write.
...
- When editable-BOL cleanup is planned, a Chrome-launch preflight runs before the Teamship API. A missing browser executable or unavailable private virtual display is a worker-preflight failure; no API fields or pallet rows are written. The production Garland service uses Xvfb rather than the VNC desktop, avoiding stale `DISPLAY` and Xauthority session cookies.
...
- Garland review now uses that same read-session recovery for API list/detail requests. If Teamship rejects a token during a batch, only the failed read is repeated after one renewal; successful earlier reads are retained. A rejected renewal, repeated 401, or 403 is propagated through the dashboard/archive handlers instead of being treated as an unavailable search source or a missing order. Regression coverage includes failures before any exact matches and after a partial match. The cause of Teamship invalidating a particular live token requires Teamship-side evidence; the application does not assume that the saved password is wrong.
