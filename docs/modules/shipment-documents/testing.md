...
- Teamship browser-preflight tests prove that only eligible BOL-cleanup plans require Chrome, a missing or stale display is reported without raw browser logs, and the Teamship API callback is never invoked when Chrome cannot start. VM-service tests require the Garland worker to use Xvfb and require installation to stop before changing services when the dependency is unavailable.
...
- TMG pickup regression tests require matching positive pickup evidence in both the picklist and the exact order packet, preserve the full pickup packet for upload, omit freight-only BOL/label/PRO requirements, and prove that ordinary freight with missing documents remains blocked.
- TMG batch-reprocessing route tests retain authenticated module and mutation checks; the service permits only unapproved, unstarted, tenant-scoped batches and keeps CSR approval separate from preparation.
- TMG backlog regression tests keep the Graph metadata window large while selecting only the oldest tenant-and-mailbox-unsaved candidate for PDF processing in each bounded scan.
...
- TMG Teamship-create tests require warehouse instructions to be appended to both Shipment ID and PO Number, set Pickup ETA to the next US business day after receipt using the US Eastern calendar and observed federal holidays, keep plain references when instructions are absent, preserve exact-reference matching for duplicate and readback checks, recover bounded transient read failures, and prove the create request remains single-attempt. TMG PDF tests cover fragmented Estes BOL text, exact PRO-to-picklist fallback matching, and rejection of conflicting readable references. TMG document-upload tests cover both automatic upload and the staged-file Save/Upload flow, require the asynchronous upload response and network-idle wait before reload, and fail closed on missing or ambiguous submit controls and missing or rejected upload requests. The employee PDF-review route is tenant/batch/order-scoped and verifies the PDF signature and saved hash before returning bytes.
