...
TMG order intake is documented separately at `docs/customers/tmg/overview.md`. Its implementation is under `src/app/(authenticated)/operations/tmg-order-intake`, `src/app/api/operations/tmg-order-intake`, `src/modules/shipment-documents/tmg-*`, and `scripts/tmg-teamship-order-worker.ts`. Its isolated VM runtime is defined by `ops/teamship-phase2-vm/newl-tmg-order-intake-worker.service`, `ops/teamship-phase2-vm/tmg-order-intake-worker.env.example`, and the shared VM installer/updater scripts.
...
| Carrier manifests | `src/modules/shipment-documents/carrier-manifest-*`, `src/modules/shipment-documents/components/garland-carrier-manifest-client.tsx`, `src/app/api/shipment-documents/carrier-manifest/*` | `ShipmentCarrierManifestRun` | `tests/carrier-manifest-*.test.ts` |
