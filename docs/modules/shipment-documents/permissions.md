# Shipment documents and Garland Teamship review: Permissions
...
- Manual Garland Email Intake review requires an authenticated tenant membership, Shipment Documents module access, mutation permission, an exact confirmation phrase enforced by the server, and tenant-scoped attachment and PS-range validation. Both the request and its aggregate outcome are audited. The action can invoke only the existing supported Garland Teamship update preparation and cannot authorize printing.
