...
- The production-preview server action on the Identity Review page accepts exactly one tenant-owned active operating company plus the explicit `RUN_READ_ONLY_PREVIEW` confirmation. It delegates to `runCustomerIntelligenceDryRun`, so the ADMIN-only `requireIngestionAdmin` and tenant mutation gate remain authoritative. The control is hidden from non-ADMIN roles, and the server guard still denies direct invocation.
