...
- `tests/customer-intelligence-dry-run.test.ts` — CP-PHASE-02B-7 consolidated end-to-end dry-run verification suite. Prisma is mocked; the authorization module is REAL; `fetch` is mocked. Covers:
  ...
  - **Safe reconciliation diagnostics**: failures reading operating-company relationships, canonical-company candidates, and approved mappings are converted to bounded count-only classifications; known Prisma missing-table/missing-column codes receive explicit schema classifications. Raw database messages, customer identifiers, email addresses, and credentials are excluded, every affected record remains an error, and the dry run performs no Customer Intelligence writes.
