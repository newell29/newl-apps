...
- `DevelopmentSuggestion` is a focused approval queue. Similar feedback is grouped before review. Selecting **Approve & queue Rivet** records the decision and queues a restricted local Codex branch-and-PR job; if earlier work in the same module/workflow is active or review-blocked, the new job waits without losing approval. It does not authorize merge, deployment, migration execution, Teamship writes, printing, shipping, permission changes, or customer communication.
