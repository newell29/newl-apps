...
The signed-in exact-order Teamship page is authoritative for print-plan pallet-label quantity because it is the same order screen the local worker preflights immediately before printing. The tenant-bound local browser-read worker must confirm the exact internal page ID, Garland customer scope, Annagem warehouse scope, and one valid hidden `pallets_count` value before an approval request can exist. API detail and list-summary aliases are not a fallback for this approval quantity. If the local page cannot be read or does not match, plan creation fails closed. During the supervised `30666` investigation on 2026-07-22, the signed-in Teamship page showed one pallet while the API could still report `2`; plans using the stale value were rejected.
