...
Exact Garland PS/SR lookups use Teamship's authenticated **Open** dashboard search, restricted server-side to Garland rows, before applying exact PS/SR matching. The dashboard request follows Teamship's Syncfusion contract: `POST /api/ship-inventories/dashboard` with search, paging, count, and status values in the JSON body. After web sign-in, Newl Apps reloads the authenticated shipping-order page and uses that page's CSRF token and refreshed cookies for the dashboard request; the pre-login form token is not reused after Teamship regenerates the session. Dashboard pages are capped at 100 rows to stay within the size exposed by Teamship's interface. This avoids walking more than 21,000 unrelated active shipping orders and exceeding the review execution limit. If the dashboard request itself fails, the older active API scan is capped at 1,000 rows as a compatibility fallback so a degraded lookup cannot consume the full execution window. Teamship removes an order from the active API feed after warehouse staff physically pick and complete it. The explicit **Recheck Teamship** recovery action searches Open first, then uses the authenticated read-only Complete dashboard archive only for still-missing references. This preserves later manual rechecks without treating completion as the cause of an earlier active-order miss. Each dashboard source remains bounded, and lookup stops as soon as every requested PS/SR pair is confirmed.
