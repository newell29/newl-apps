...
Exact Garland PS/SR lookups have a separate bounded Teamship pagination allowance from the ordinary daily-order listing. This is necessary because Teamship's shipping-order list can place current orders beyond the first 15,000 returned rows even when descending ordering is requested. Teamship removes an order from the active API feed after warehouse staff physically pick and complete it. The normal automatic review reads only the active feed. The explicit **Recheck Teamship** recovery action scans the active feed first, then uses the authenticated read-only Complete dashboard archive only for still-missing references. This preserves later manual rechecks without treating completion as the cause of an earlier active-order miss. Each source is bounded to at most 100 capped pages (50,000 rows at the default page size), and lookup stops as soon as every requested PS/SR pair is confirmed.
