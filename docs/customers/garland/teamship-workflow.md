...
The read-only Teamship comparison uses one renewable API session per lookup. If an order-detail or fallback-list read returns 401 after login, it renews the token once and retries only that failed read; later reads use the renewed token and previously collected exact matches are retained. A rejected renewal, second 401, or 403 stops the comparison rather than falling back with an invalid token or reporting missing/partial orders. The existing bounded transient-read retries also apply. Initial credential rejection is not retried. This recovery cannot create or replay a Teamship update, BOL cleanup, print job, or shipment release.
