...
The developer-operated [AI Development Engine](ai-development-engine.md) is a separate experiment for supervised Qwen planning, DeepSeek implementation, deterministic verification, and independent Qwen review. Version 1B.1 adds the local `npm run ai:feature` operator launcher, credential-free user model defaults, ignored feature state, exact single-phase approvals, structured owner decisions, progress events, safe review recovery, exact resumable correction boundaries, and a small loopback-only operator UI. Confirmed owner evidence retains both the selected answer and its optional explanation through phase generation, implementation, evaluation, independent review, and recovery. The Version 1B.1.1 compatibility path imports explicitly documented stable questions from legacy workflows without another planner call and keeps their owner-gated phase blocked. The planner-resilience patch uses a bounded two-turn planning session, one same-session structured repair, immediate provider-identity capture, redacted owner-only diagnostics, and a fail-closed `recover-plan` boundary that never starts a builder. The UI can show status, answer hashed owner questions, approve one phase, and resume a pinned correction, but it is not hosted or remotely reachable. The engine remains local, sequential, and fail-closed; the linked architecture is the retained long-term roadmap rather than permission to add deployment or production actions.
