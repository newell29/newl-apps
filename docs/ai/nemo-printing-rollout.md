...
The local browser-read and print workers must open the shipping-order detail UI on `members.fulfillit.io`. Both use `TEAMSHIP_APP_BASE_URL`; set it explicitly in their protected local environment instead of deriving the web host from the API host. Newl Apps sends an approval-plan preflight through the existing tenant-bound browser-read queue. The local signed-in worker must see the exact `/ship-inventories/:id` page and the configured customer and warehouse. On editable orders, Teamship's hidden `pallets_count` is only the number of pallet-size rows; the worker requires that many observed rows and sums their physical pallet quantities. For completed orders it reads the bounded static pallet table. It rejects missing, ambiguous, invalid, mismatched, or default-only rows before Newl Apps creates a print request.
...
For approval-plan and print-execution pallet counts, the signed-in shipping-order page is authoritative. API detail and list-summary pallet aliases may resolve the order identity but never substitute for or override the local page count. Before any document is submitted, the print worker also opens the exact Step 2 menu and outbound-label dialog, validates the unique quantity and Print controls, and closes the dialog without printing. It reads the page count again immediately before the irreversible outbound-label action. The initial plan identity lookup must not perform a separate server-side Teamship web login because the local signed-in browser performs the authoritative page preflight.
