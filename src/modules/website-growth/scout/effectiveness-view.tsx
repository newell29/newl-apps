...
export function EffectivenessReview({ review, items, mission, canReview, workspaceAvailable, truncated, competitorSummary }: {
  ...
  return <>
    <section className={`${panel} space-y-3`}>
      <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold">Direction and coverage</h2>{canReview && <form action={refreshAction}><button className={button} disabled={pending}>{pending ? "Checking saved sources…" : "Check review freshness"}</button></form>}</div>
      ...
      <p className="text-sm">{review ? `Comparing ${review.windows.after.startDate}–${review.windows.after.endDate} with ${review.windows.before.startDate}–${review.windows.before.endDate}. Last refresh attempt ${date(review.attemptedAt)}. Next eligible refresh ${date(review.nextRefreshAt)}.` : "No site review is saved yet. An enabled Scout wake collects it automatically. An owner can check freshness now without running an AI research step."}</p>
