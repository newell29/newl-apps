...
export function EffectivenessReview({ review, items, mission, canReview, workspaceAvailable, truncated, competitorSummary }: {
  ...
  return <>
    ...
    <aside className={`${panel} space-y-2 text-sm`}><strong>Business outcomes and attribution</strong><p>{mission?.qualifiedLead ?? "Define a qualified enquiry in the marketing direction."}</p><p>Enquiries are website forms grouped by the submitted page. Qualified, Quote sent and Won show separate current statuses recorded in Inbound Opportunities; these are not historical conversion stages or revenue. Explicit tests, Codex diagnostics and automated form checks are excluded; unmarked spam can remain.</p><p>This SEO view remains submitted-page reporting. Original paid landing attribution is available in Paid campaigns; revenue is not connected. The competitor watchlist guides task-level public research; it is not an automatic continuous crawl or evidence of competitors’ private results.</p><Link href="/website-growth/paid-campaigns" className="inline-block text-primary">Review paid campaign attribution →</Link><br /><Link href="/website-inbound" className="inline-block text-primary">Review and qualify inbound opportunities →</Link></aside>
  </>;
}
