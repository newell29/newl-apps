...
export function AuthorityBoard({ workspace, canReview }: { workspace: Awaited<ReturnType<typeof authorityWorkspace>>; canReview: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    <section className={card}><h2 className="text-lg font-semibold">{campaign?.title ?? "Start the placement pilot"}</h2>
      ...
      <p className="mt-3 text-sm"><strong>Research:</strong> {campaign?.enabled ? "Active within the existing Scout research schedule and budget." : "Paused."} <strong>Executor:</strong> {wake ? `Last check ${wake.at?.toISOString() ?? "unknown"}. ${String(record(wake.output).reason ?? "")}` : "No campaign executor check recorded. Runtime cutover is still required."}</p>
