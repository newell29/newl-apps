  ...
  try {
    ...
    if (typeof body.runId !== "string" || !body.runId.trim()) {
      return NextResponse.json(
        { error: "Hunter Luna synthesis runId is required." },
        { status: 400 }
      );
    }
    if (!Array.isArray(body.packets)) {
      return NextResponse.json(
        { error: "Hunter Luna synthesis packets must be an array." },
        { status: 400 }
      );
    }
    ...
    const status = error instanceof IngestionAuthError
      ...
      : error instanceof SyntaxError
        ...
        : 422;
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Hunter Luna synthesis failed." },
      { status }
    );
  }
}
