...
import { completeWebsiteGrowthBacklinkDiscovery } from "@/modules/website-growth/backlink-discovery";
...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { runId?: unknown; decisions?: unknown };
    if (typeof body.runId !== "string" || !body.runId.trim()) {
      return NextResponse.json({ error: "Website Growth Scout runId is required." }, { status: 400 });
    }
    const data = await completeWebsiteGrowthBacklinkDiscovery({
      tenantId: tenant.id,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Backlink discovery completion failed." },
      { status }
    );
  }
}
