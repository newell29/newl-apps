  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Work the people Hunter has selected for contact discovery, message preparation, Apollo enrollment, and follow-up. Positive engagement graduates to Sales Opportunities."
      />
        ...
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border bg-muted px-4 py-3">
          <div>
            ...
            <p className="text-xs text-mutedForeground">
              Narrow the active queue by company, search profile, outreach state, draft readiness, and ownership.
            </p>
          </div>
...
function buildContactsPageHref(params: Record<string, string | undefined>) {
  ...
  return query.length > 0 ? `/lead-gen/outreach?${query}` : "/lead-gen/outreach";
}
...
function buildContactsExportHref(params: Record<string, string>) {
  ...
  return `/api/lead-gen/contacts/export?${searchParams.toString()}`;
}
