...
export default async function WebsiteGrowthPage({
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Website Growth · Research"
        title="Research signals"
        description="Raw GA4, Search Console, Semrush, and Newl Apps evidence used by Scout. These records are research inputs—not approved ideas or active website work."
      />
      ...
      <nav className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-card p-2 shadow-sm" aria-label="Website Growth views">
        ...
        <p className="ml-auto hidden px-3 text-xs text-mutedForeground lg:block">
          Scout decides which signals become reviewable ideas.
        </p>
      </nav>

      <section className="rounded-lg border border-accentBorder bg-accentSoft p-5">
        <p className="text-xs font-semibold uppercase tracking-wide text-primary">Supporting evidence only</p>
        <h2 className="mt-2 text-lg font-semibold text-foreground">
          These {shell.metrics.totalCount.toLocaleString("en-US")} records are not your active work queue.
        </h2>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-mutedForeground">
          Scout reviews, combines, and removes duplicates from these signals. Return to the Scout workspace to see the small set of ideas that actually need a decision.
        </p>
      </section>

      <section className="grid gap-4 md:grid-cols-4">
        <MetricCard label="Stored signals" value={shell.metrics.totalCount} caption="All research evidence" />
        <MetricCard label="New / reviewing" value={shell.metrics.reviewQueueCount} caption="Candidates Scout can assess" />
        <MetricCard label="Monitoring" value={shell.metrics.monitoringCount} caption="Retained without active work" />
      ...
      <section className="rounded-lg border border-border bg-card p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-base font-semibold text-foreground">Research filters</h2>
            <p className="mt-1 text-sm leading-6 text-mutedForeground">
              Search the underlying analytics evidence by status, action, keyword, page, or reason.
            </p>
          </div>
          <Link
            href="/website-growth/signals"
      ...
      <section className="rounded-lg border border-border bg-card shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border p-5">
          <div>
            <h2 className="text-base font-semibold text-foreground">Raw signal inventory</h2>
            <p className="mt-1 text-sm leading-6 text-mutedForeground">
...
// Retained for older template-generated records that may return to the research view.
