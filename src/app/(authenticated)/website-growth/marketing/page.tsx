...
function WorkCard({ item, canReview }: { item: Work & { id: string; recipientEmail?: string | null }; canReview: boolean }) {
  ...
  return <article className="rounded-lg border border-border bg-card p-4 shadow-sm">
    ...
    {item.state === "READY" && supervisor.verdict === "REVISE" && <p className="mt-2 text-xs text-mutedForeground">Correction ready for the next available research wake. The research budget and active-work limit still apply.</p>}
