...
export async function extractPdfTextPagesFromBytes(fileBytes: Uint8Array) {
...
async function ensurePdfJsNodeWorkerGlobal() {
  ...
  if (globals.pdfjsWorker?.WorkerMessageHandler) return;
...
class PdfJsDomMatrixShim {
  ...
  translate(x = 0, y = 0) {
    return new PdfJsDomMatrixShim(this).multiplySelf(
      new PdfJsDomMatrixShim([1, 0, 0, 1, x, y])
    );
  }

  scale(scaleX = 1, scaleY = scaleX) {
    return new PdfJsDomMatrixShim(this).multiplySelf(
      new PdfJsDomMatrixShim([scaleX, 0, 0, scaleY, 0, 0])
    );
  }
