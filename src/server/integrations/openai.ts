...
export async function reviewOutreachSequenceGrounding(
  ...
  const response = await requestStructuredOpenAiResponse({
    ...
    system:
      "You are the conservative QA gate for Newl Group outbound logistics messaging. Inspect only customer-visible subject/body text and the human-visible call-task instruction for prohibited outbound wording. Internal sequence names, evidenceRefs, evidence-ledger IDs, database labels, and other grounding metadata are not outbound copy and must never be reported as outbound violations. Compare every factual claim in the customer-visible copy with the saved evidence ledger. Newl capabilities explicitly listed in a NEWL_CAPABILITY evidence record are approved facts and must not be rejected merely because they are not company research. Fail unsupported events, shipment claims, quantities, geography, buyer responsibilities, incumbent-provider assumptions, unlisted Newl capabilities, deceptive familiarity, or messages that imply a LinkedIn/call action already happened. Do not let a date, quantity, or intended outcome from one fact modify a separate fact. A job posting supports only recruitment for the named role, not added capacity, team growth, increased workload, or an operational problem. Also fail any customer-visible use of the word Hunter, any internal evidence ID or research-system reference, any sender placeholder or generic Newl Group signature, any email that does not end with senderFirstName on its own final line, and any mismatch between allowCallTask and the actual outbound channels. The supplied outboundSequence schedule is authoritative; do not invent or enforce different timing from strategy prose. Style preferences alone are warnings. Return passed=false whenever any ERROR exists.",
    user: JSON.stringify({
      companyName: context.companyName,
      contact: context.contact,
      strategyContext: {
