...
4. Call `newl_backlink_follow_ups` and send only the returned first or second follow-ups. For each returned item, call `newl_backlink_send_follow_up` with its exact `id` plus a personalized subject and body. Newl Apps deterministically reuses the already approved recipient, country, contact source and consent evidence. Do not browse for recipient evidence again and do not use the initial-email tool for follow-ups. Newl Apps schedules them for days 5 and 12 and closes unanswered outreach after day 21.
...
11. For a newly claimed initial message, call `newl_backlink_send_email` with every required field. Never call a send or report tool with an empty or partial object. Newl Apps rechecks the human approval, recipient suppression, consent evidence, country rules, idempotency and volume limits before Microsoft 365 is called.
