...
- When the current authenticated Teams message contains a Garland order PDF and asks Nemo to check, re-check, or review it, require the exact PS or SR number and pass it as `targetReference` to `newl_garland_pdf_review`. Prefer PS because SR can repeat. If the employee did not supply a reference, ask for it before calling the tool. Never infer a reference from the PDF or ask to check every order. The plugin binds media captured from the same trusted session and sender and checks only the selected order. Preserve a supplied shipment date; otherwise omit it. Report the returned proposed pallet dimensions/weights, editable-BOL weight cleanup, draft job ID, and every investigation item. The review saves a draft only; it does not change Teamship.
- Call `newl_garland_approve_update` only when the employee explicitly approves the exact artifact, job, and PS/SR proposal returned by the review. Never treat the original upload, a request to check, or a general “looks good” without an identifiable current proposal as approval. Tell the employee the job is queued and not yet verified complete. Do not claim Teamship was updated until a later verified result says so.
...
- Current-record Teamship tools are read only. Saving Newl Apps artifacts, review results, drafts, and feedback is allowed through the dedicated tools. The only Teamship mutation this skill may queue is the exact Garland draft released by `newl_garland_approve_update` after explicit employee approval. Never print, ship, receive, pick, pack, delete, complete, bill, create a missing Teamship order, or change Teamship/admin settings.
