...
while IFS= read -r worker_env_line || [[ -n "${worker_env_line}" ]]; do
  ...
  case "${worker_env_name}" in
    NEWL_APPS_BASE_URL|INGESTION_API_TOKEN|INGESTION_TENANT_SLUG|TRADEMINING_USER|TRADEMINING_PASSWORD|HUNTER_WORKER_ID|HUNTER_PYTHON_PATH|HUNTER_EXPORT_DIRECTORY|HUNTER_PROCESSED_DIRECTORY|HUNTER_COOKIE_FILE|HUNTER_TRADEMINING_PORTS_JSON|HUNTER_HTTP_MAX_ATTEMPTS|HUNTER_DAILY_RUN_TIME|HUNTER_END_DATE|HUNTER_POLL_MS|HUNTER_SIGNAL_SCOUT_ENABLED|HUNTER_SIGNAL_SCOUT_DAILY_TIME|HUNTER_SIGNAL_SCOUT_TIMEZONE|HUNTER_OLLAMA_BASE_URL|HUNTER_CLASSIFICATION_MODEL|HUNTER_CLASSIFICATION_BATCH_SIZE|HUNTER_COMPANY_RESEARCH_ENABLED|HUNTER_COMPANY_RESEARCH_DAILY_TIME|HUNTER_COMPANY_RESEARCH_TIMEZONE|HUNTER_RESEARCH_SEARCH_PROVIDER|HUNTER_BRAVE_SEARCH_API_KEY|HUNTER_RESEARCH_RESULTS_PER_QUERY|HUNTER_RESEARCH_PAGES_PER_COMPANY|HUNTER_RESEARCH_FOLLOW_UP_QUERIES|HUNTER_RESEARCH_QWEN_MODEL|HUNTER_RESEARCH_QWEN_BATCH_SIZE|HUNTER_KIMI_API_KEY|HUNTER_KIMI_BASE_URL|HUNTER_KIMI_MODEL|HUNTER_RESEARCH_KIMI_BATCH_SIZE|VERCEL_AUTOMATION_BYPASS_SECRET)
