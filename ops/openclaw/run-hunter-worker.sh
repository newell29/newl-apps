...
while IFS= read -r worker_env_line || [[ -n "${worker_env_line}" ]]; do
  ...
  case "${worker_env_name}" in
    NEWL_APPS_BASE_URL|INGESTION_API_TOKEN|INGESTION_TENANT_SLUG|TRADEMINING_USER|TRADEMINING_PASSWORD|HUNTER_WORKER_ID|HUNTER_PYTHON_PATH|HUNTER_EXPORT_DIRECTORY|HUNTER_PROCESSED_DIRECTORY|HUNTER_COOKIE_FILE|HUNTER_TRADEMINING_PORTS_JSON|HUNTER_HTTP_MAX_ATTEMPTS|HUNTER_DAILY_RUN_TIME|HUNTER_END_DATE|HUNTER_POLL_MS|HUNTER_SIGNAL_SCOUT_ENABLED|HUNTER_SIGNAL_SCOUT_DAILY_TIME|HUNTER_SIGNAL_SCOUT_TIMEZONE|HUNTER_OLLAMA_BASE_URL|HUNTER_CLASSIFICATION_MODEL|HUNTER_CLASSIFICATION_BATCH_SIZE|VERCEL_AUTOMATION_BYPASS_SECRET)
