...
while IFS= read -r worker_env_line || [[ -n "${worker_env_line}" ]]; do
  ...
  case "${worker_env_name}" in
    NEWL_APPS_BASE_URL|TEAMSHIP_BROWSER_WORKER_TOKEN|TEAMSHIP_BROWSER_WORKER_ID|TEAMSHIP_BROWSER_EXECUTABLE_PATH|TEAMSHIP_BROWSER_READ_HEADED|TEAMSHIP_BROWSER_READ_TIMEOUT_MS|TEAMSHIP_BROWSER_WORKER_POLL_MS|TEAMSHIP_BROWSER_ALLOWED_HOSTS|TEAMSHIP_WORKER_REPO_PATH|TEAMSHIP_APP_BASE_URL|VERCEL_AUTOMATION_BYPASS_SECRET)
...
if [[ ! -f "${worker_repo_path}/package.json" ]]; then
  echo "Teamship browser worker repository path is invalid." >&2
  exit 1
fi

cd "${worker_repo_path}"
exec /opt/homebrew/bin/npm run worker:teamship-browser-read
