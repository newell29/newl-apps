...
describe("Authority control centre walkthrough", () => {
  ...
  it("shows the exact proposal, concrete blockers, receipt and live evidence separately", () => {
    ...
    const w = workspace(); w.actions = [{ id: "review", ...base }, { id: "hold", ...base, state: "UNCERTAIN", result: "Reconcile publisher receipt" },
    ...
    expect(readOnly).not.toContain("Approve exact action"); expect(readOnly).not.toContain("Save campaign"); expect(readOnly).not.toContain("Record reconciliation and close");
  });
});
